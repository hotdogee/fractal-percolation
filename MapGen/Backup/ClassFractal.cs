...
namespace MapGen
{
	...
	public class FractionalBrownianMotion
	{
		...
		public void SetProperties(int width, int height)
		{
			Width = width;
			Height = height;
			_byteMap = new byte[_width, _height];
			_realMap = new double[width, height];
		}
        ...
        private void _spectralSynthesis2D_FFTW()
        {
            ...
            IntPtr pin = fftw.malloc(sizeof(double) * 2 * _height * _width);
            IntPtr pout = fftw.malloc(sizeof(double) * 2 * _height * _width);

            // managed arrays
            // n*2 because we are dealing with complex numbers
            double[] fin = new double[_height * _width * 2];
            double[] fout = new double[_height * _width * 2];
            ...
            for (int i = 0; i <= halfHeight; i++)
            {
                for (int j = 0; j <= halfWidth; j++)
                {
                    ...
                    if (i == 0)
                        ...
                        i0 = Convert.ToInt32(_height) - i;
                    if (j == 0)
                        j0 = 0;
                    else
                        j0 = Convert.ToInt32(_width) - j;
