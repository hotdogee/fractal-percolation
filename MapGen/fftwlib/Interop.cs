...
namespace fftwlib
{
	...
	public class fftw
    {
        ...
        private static Mutex mut_dft_2d = new Mutex();
        //private static Mutex mut_execute = mut_dft_2d;
        private static Mutex mut_malloc = mut_dft_2d;
        private static Mutex mut_free = mut_dft_2d;
        private static Mutex mut_destroy_plan = mut_dft_2d;
		/// <summary>
		/// Allocates FFTW-optimized unmanaged memory
		/// </summary>
		/// <param name="length">Amount to allocate, in bytes</param>
        ...
        public static extern IntPtr malloc_extern(IntPtr length);
        [DllImport("libfftw3-3-x64.dll", EntryPoint = "fftw_malloc", ExactSpelling = true)]
        public static extern IntPtr malloc_extern_x64(IntPtr length);

        public static IntPtr malloc(IntPtr length)
        {
            IntPtr plan;
            mut_malloc.WaitOne();
            if (is64BitProcess)
            {
                plan = malloc_extern_x64(length);
            }
            else
            {
                plan = malloc_extern(length);
            }
            mut_malloc.ReleaseMutex();
            return plan;
        }
        ...
        public static extern void free_extern(IntPtr mem);
        [DllImport("libfftw3-3-x64.dll", EntryPoint = "fftw_free", ExactSpelling = true)]
        public static extern void free_extern_x64(IntPtr mem);
        ...
        public static extern void destroy_plan_extern(IntPtr plan);
        [DllImport("libfftw3-3-x64.dll", EntryPoint = "fftw_destroy_plan", ExactSpelling = true)]
        public static extern void destroy_plan_extern_x64(IntPtr plan);

        public static void destroy_plan(IntPtr plan)
        {
            mut_destroy_plan.WaitOne();
            if (is64BitProcess)
            {
                destroy_plan_extern_x64(plan);
            }
            else
            {
                destroy_plan_extern(plan);
            }
            mut_destroy_plan.ReleaseMutex();
            return;
        }
        ...
        public static extern void execute_extern(IntPtr plan);
        [DllImport("libfftw3-3-x64.dll", EntryPoint = "fftw_execute", ExactSpelling = true)]
        public static extern void execute_extern_x64(IntPtr plan);
        ...
        public static void execute(IntPtr plan)
        {
            ...
            if (is64BitProcess)
            {
                execute_extern_x64(plan);
            }
            else
            {
                execute_extern(plan);
            }
            //mut_execute.ReleaseMutex();
            return;
        }
        ...
        public static extern IntPtr dft_2d_extern(int nx, int ny, IntPtr input, IntPtr output, fftw_direction direction, fftw_flags flags);
        [DllImport("libfftw3-3-x64.dll", EntryPoint = "fftw_plan_dft_2d", ExactSpelling = true)]
        public static extern IntPtr dft_2d_extern_x64(int nx, int ny, IntPtr input, IntPtr output, fftw_direction direction, fftw_flags flags);

        public static IntPtr dft_2d(int nx, int ny, IntPtr input, IntPtr output, fftw_direction direction, fftw_flags flags)
        {
            IntPtr plan;
            mut_dft_2d.WaitOne();
            if (is64BitProcess)
            {
                plan = dft_2d_extern_x64(nx, ny, input, output, direction, flags);
            }
            else
            {
                plan = dft_2d_extern(nx, ny, input, output, direction, flags);
            }
            mut_dft_2d.ReleaseMutex();
            return plan;
        }
