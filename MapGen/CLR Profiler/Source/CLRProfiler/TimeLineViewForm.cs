...
namespace CLRProfiler
{
    ...
    public class TimeLineViewForm : System.Windows.Forms.Form
    {
        ...
        private void AddAddress(uint addr)
        {
        ...
        private int AddrToY(uint addr)
        {
            int y = topMargin;
            for (AddressRange r = rangeList; r != null; r = r.next)
            {
                if (r.loAddr <= addr && addr <= r.hiAddr)
                    return y + (int)((r.hiAddr - addr)/verticalScale);
                y += (int)((r.hiAddr - r.loAddr)/verticalScale);
        ...
        private void DrawChangeList(Graphics g, SampleObjectTable.SampleObject so, uint addr)
        {
        ...
        private void DrawSamples(Graphics g, SampleObjectTable.SampleObject[][] masterTable)
        {
            ...
            for (AddressRange r = rangeList; r != null; r = r.next)
            {
                ...
                {
                    uint hiAddr = YToAddr(rangeTop);
                    for (uint addr = (uint)(YToAddr(rangeBottom)/verticalScale*verticalScale); addr <= hiAddr; addr += (uint)verticalScale)
                    {
                        uint i = addr >> SampleObjectTable.firstLevelShift;
                        SampleObjectTable.SampleObject[] soa = masterTable[i];
                        if (soa != null)
                        {
                            uint j = (addr >> SampleObjectTable.secondLevelShift) & (SampleObjectTable.secondLevelLength-1);
                            SampleObjectTable.SampleObject so = soa[j];
                            if (so != null)
                            {
                                int y = AddrToY(addr);
        ...
        private void DrawAddressLabel(Graphics g, Brush brush, Pen pen, AddressRange r, int y, uint addr)
        {
            y += (int)((r.hiAddr - addr)/verticalScale);
        ...
        private void DrawAddressLabels(Graphics g)
        {
            ...
            uint minLabelPitch = (uint)(minLabelPitchInPixels*verticalScale);

            uint labelPitch = 1024;
            ...
            for (AddressRange r = rangeList; r != null; r = r.next)
            {
                ...
                for (uint addr = (r.loAddr + labelPitch*3/2) & ~(labelPitch-1); addr <= r.hiAddr; addr += labelPitch)
        ...
        private int NextLabelX(Graphics g, SampleObjectTable.SampleObject gc, int gen, int gcCount)
        {
            if (gen < 2)
            {
                for (gc = gc.prev; gc != null; gc = gc.prev)
                {
                    ...
                    if (gc.typeIndex > gen)
                    {
                        ...
                        string s = string.Format("gc #{0} (gen {1})", gcCount, gc.typeIndex);
        ...
        private void DrawCommentLines(Graphics g)
        {

            Pen pen = new Pen(Color.LimeGreen);
            for (SampleObjectTable.SampleObject comment = sampleObjectTable.commentList; comment != null; comment = comment.prev)
            {
                int x = TimeToX(lastLog.TickIndexToTime(comment.allocTickIndex));
                g.DrawLine(pen, x, 0, x, graphPanel.Height);
            }
        }
        ...
        private void graphPanel_Paint(object sender, System.Windows.Forms.PaintEventArgs e)
        {
            ...
            verticalScale = Scale(verticalScaleGroupBox, availablePixels, (int)(range/1024))*1024;
            ...
            for (AddressRange r = rangeList; r != null; r = r.next)
            {
                height += (int)((r.hiAddr - r.loAddr)/verticalScale);
        ...
        private void CalculateAllocatedObjectSizes(int startTickIndex, int endTickIndex, uint addr)
        {
            uint index = addr >> SampleObjectTable.firstLevelShift;
        ...
        private void CalculateAllocatedObjectSizes(int startTick, int endTick)
        {
            for (AddressRange r = rangeList; r != null; r = r.next)
            {
                for (uint addr = r.loAddr; addr < r.hiAddr; addr += SampleObjectTable.sampleGrain)
                {
        ...
        private uint YToAddr(int y)
        {
            int rY = topMargin;
            for (AddressRange r = rangeList; r != null; r = r.next)
            {
                int nextY = rY + (int)((r.hiAddr - r.loAddr)/verticalScale);
                if (rY <= y && y <= nextY)
                    return r.hiAddr - (uint)((y - rY)*verticalScale);
                rY = nextY;
        ...
        private void DrawChangeList(Graphics g, SampleObjectTable.SampleObject so, uint addr, int tickIndex)
        {
            ...
            for (uint i = 0; i < masterTable.Length; i++)
            {
                ...
                if (soa != null)
                {
                    for (uint j = 0; j < soa.Length; j++)
                    {
                        ...
                        if (so != null)
                        {
                            uint addr = (i<<SampleObjectTable.firstLevelShift)
                                      + (j<<SampleObjectTable.secondLevelShift);
        ...
        SampleObjectTable.SampleObject FindSampleObject(int tickIndex, uint addr)
        {
            uint index = addr >> SampleObjectTable.firstLevelShift;
            SampleObjectTable.SampleObject[] sot = sampleObjectTable.masterTable[index];
            if (sot == null)
        ...
        private string FindComment(int mouseX)
        {
            ...
            for (SampleObjectTable.SampleObject comment = sampleObjectTable.commentList; comment != null; comment = comment.prev)
            {
                int commentX = TimeToX(lastLog.TickIndexToTime(comment.allocTickIndex));
                int dist = Math.Abs(commentX - mouseX);
                if (resultDist > dist)
                {
                    resultDist = dist;
                    result = sampleObjectTable.readNewLog.commentTable[comment.typeIndex];
                }
            }
            return result;
        }
