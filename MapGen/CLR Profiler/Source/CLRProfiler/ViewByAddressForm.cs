...
namespace CLRProfiler
{
    ...
    public class ViewByAddressForm : System.Windows.Forms.Form
    {
        ...
        class AddressRange
        {
            ...
            internal int index;
            internal uint[] genLoAddr;
            internal uint[] genHiAddr;

            internal AddressRange(uint loAddr, uint hiAddr, AddressRange next, int index)
            {
                this.loAddr = loAddr;
                this.hiAddr = hiAddr;
                this.next = next;
                this.index = index;
                this.genLoAddr = new uint[generations];
                this.genHiAddr = new uint[generations];
                for (int i = 0; i < generations; i++)
                {
                    this.genLoAddr[i] = uint.MaxValue;
                    this.genHiAddr[i] = uint.MinValue;
                }
            }
        }
        ...
        void BuildAddressRangesTypeTable()
        {
            ...
            for (liveObjectTable.GetNextObject(0, uint.MaxValue, out o); o.id < uint.MaxValue; liveObjectTable.GetNextObject(o.id + o.size, uint.MaxValue, out o))
            {
        ...
        private AddressRange AddressRangeOf(uint addr)
        {
            AddressRange r;
        ...
        private void FillSpace(Graphics g, AddressRange r, Pen pen, uint start, uint end)
        {
            // and a relative address of the object in this range
            uint relativeStartAddr = start - r.loAddr;
            uint relativeEndAddr = end - r.loAddr;
        ...
        private uint VisibleYToAddress(AddressRange r, int y)
        {
            return r.loAddr + (uint)((graphPanel.Size.Height - bottomMargin - y + graphPanel.Top)*heapWidth*bytesPerPixel);
        }

        private int InsideSelection(uint addr)
        {
        ...
        void IntersectIntervals(uint aLow, uint aHigh, ref uint bLow, ref uint bHigh)
        {
            bLow = Math.Max(aLow, bLow);
            bHigh = Math.Min(aHigh, bHigh);
        }

        private void DrawLiveObjects(Graphics g, TypeDesc selectedType, uint lowAddr, uint highAddr)
        {
            ...
            for (AddressRange r = rangeList; r != null; r = r.next)
            {
                ...
                if (x2 < clipRect.Left || clipRect.Right < x1)
                    continue;
                uint visibleStartAddr = VisibleYToAddress(r, outerGraphPanel.Size.Height);
                uint visibleEndAddr = VisibleYToAddress(r, 0);
                ...
                LiveObjectTable.LiveObject o;
                uint addr = liveObjectTable.FindObjectBackward(visibleStartAddr);
        ...
        private void DrawHeapAddress(Graphics g, Brush brush, Pen pen, AddressRange r, uint addr)
        {
            int baseX = leftMargin + r.index*(heapWidth + gap);
            ...
            // and a relative address of the object in this range
            uint relativeAddr = addr - r.loAddr;

            // divide the relative address by bytesPerPixel and heapWidth
            int yAddr = (int)(relativeAddr / (bytesPerPixel*heapWidth));
        ...
        private void DrawHeapLegend(Graphics g)
        {
            ...
            for (AddressRange r = rangeList; r != null; r = r.next)
            {
                ...
                uint addrPitch = (uint)(pixelPitch*bytesPerPixel*heapWidth);

                DrawHeapAddress(g, brush, pen, r, r.loAddr);

                for (uint addr = (r.loAddr + addrPitch*3/2)/addrPitch*addrPitch; addr < r.hiAddr; addr += addrPitch)
                {
        ...
        private void DrawGenerationLimits(Graphics g)
        {
            ...
            for (AddressRange r = rangeList; r != null; r = r.next)
            {
                ...
                for (int gen = 0; gen < generations; gen++)
                {
                    ...
                    {
                        uint relativeLoAddr = r.genLoAddr[gen] - r.loAddr;
                        uint relativeHiAddr = r.genHiAddr[gen] - r.loAddr;
                        int yLoAddr = (int)(relativeLoAddr / (bytesPerPixel*heapWidth));
                        int yHiAddr = (int)(relativeHiAddr / (bytesPerPixel*heapWidth));
        ...
        private void graphPanel_Paint(object sender, System.Windows.Forms.PaintEventArgs e)
        {
            ...
            for (AddressRange r = rangeList; r != null; r = r.next)
            {
                uint rangeSize = r.hiAddr - r.loAddr;
                if (maxRangeSize < rangeSize)
                    maxRangeSize = rangeSize;
            }

            bytesPerPixel = BytesPerPixel(graphPanel.Height - topMargin - bottomMargin, (int)(maxRangeSize/heapWidth));
            
            int maxHeight = (int)(maxRangeSize/(bytesPerPixel*heapWidth));
            ...
            DrawLiveObjects(g, null, 0, uint.MaxValue);
        ...
        uint selectedStartAddr, selectedEndAddr;
        uint selectedLowAddr, selectedHighAddr;

        private uint PixelCoordinatesToAddress(int x, int y)
        {
            ...
            for (AddressRange r = rangeList; r != null; r = r.next)
            {
                ...
                int minY = maxY - (int)((r.hiAddr - r.loAddr)/(bytesPerPixel*heapWidth));
                    ...
                    minY - clickMargin <= y && y <= maxY + clickMargin)
                {
                    ...
                    uint addr = r.loAddr + (uint)((y*heapWidth + x)*bytesPerPixel);
        ...
        private void DrawLiveObjectInterval(Graphics g, uint low, uint high)
        {
            if (low < high)
                DrawLiveObjects(g, null, low, high);
        }

        private void DrawLiveObjectIntervals(uint aLow, uint aHigh, uint bLow, uint bHigh)
        {
        ...
        private void EraseSelectionHorizontalLine(Graphics g, uint addr)
        {
            if (addr != 0)
            {
        ...
        private void DrawSelectionHorizontalLine(Graphics g, Pen pen, uint addr)
        {
            AddressRange r = AddressRangeOf(addr);
            if (r != null)
        ...
        private void DrawSelectionVerticalLines(Graphics g, Pen pen, uint lowAddr, uint highAddr)
        {
            ...
            // and a relative address of the object in this range
            uint relativeLowAddr = lowAddr - r.loAddr;
            uint relativeHighAddr = highAddr - r.loAddr;

            // divide the relative address by bytesPerPixel and heapWidth
            int yLowAddr = (int)(relativeLowAddr / (bytesPerPixel*heapWidth));
            int yHighAddr = (int)(relativeHighAddr / (bytesPerPixel*heapWidth));
        ...
        {
            uint oldLowAddr = selectedLowAddr;
            uint oldHighAddr = selectedHighAddr;
...
*/
        ...
        private bool FindObject(uint addr, out LiveObjectTable.LiveObject o)
        {
            uint id = liveObjectTable.FindObjectBackward(addr);
        ...
        private void graphPanel_MouseMove(object sender, System.Windows.Forms.MouseEventArgs e)
        {
            ...
            {
                uint newEndAddr = PixelCoordinatesToAddress(e.X, e.Y);
        ...
        private void RedrawType(TypeDesc t)
        {
            ...
            Graphics g = graphPanel.CreateGraphics();
            DrawLiveObjects(g, t, 0, uint.MaxValue);
            Pen pen = new Pen(Color.Black);
            DrawSelectionHorizontalLine(g, pen, selectedHighAddr);
            DrawSelectionHorizontalLine(g, pen, selectedLowAddr);
        }
        ...
        private string ComputeObjectsDescription(TypeDesc selectedType, uint selectedLowAddr, uint selectedHighAddr)
        {
            string description = "";
        ...
        private void showAllocatorsMenuItem_Click(object sender, System.EventArgs e)
        {
            ...
            uint low = selectedLowAddr;
            uint high = low == 0 ? uint.MaxValue : selectedHighAddr;
            LiveObjectTable.LiveObject o;
