...
namespace CLRProfiler
{
    ...
    internal class GraphViewForm : System.Windows.Forms.Form
    {
        ...
        private void InitializeComponent()
        {
            ...
            this.outerPanel.Size = new System.Drawing.Size(888, 565);
            ...
            // zoomToNodeMenuItem
            // 
            this.zoomToNodeMenuItem.Index = 6;
            ...
            // 
            this.findInterestingNodesMenuItem.Index = 7;
            ...
            // 
            this.showWhoAllocatedMenuItem.Index = 8;
            ...
            // 
            this.showNewObjectsMenuItem.Index = 9;
            ...
            // 
            this.showWhoAllocatedNewMenuItem.Index = 10;
            ...
            // 
            this.showObjectsAllocatedBetween.Index = 11;
            ...
            // 
            this.showWhoAllocatedObjectsBetweenMenuItem.Index = 12;
            ...
            // 
            this.showInstancesMenuItem.Index = 13;
            this.showInstancesMenuItem.Text = "Show Instances";
        ...
        string formatWeight(uint weight)
        {
...
//                            PlaceEdges(v.outgoingEdges.Values, false, v.rectangle.X + v.rectangle.Width, v.rectangle.Y, scale);
            ...
            if (filterForm.ShowDialog() == DialogResult.OK)
            {
                switch (graph.graphType)
                {
                    ...
                    case Graph.GraphType.AssemblyGraph:
                        graph = ((Histogram)graph.graphSource).BuildAssemblyGraph();
        ...
        private double Diversity(Hashtable h)
        {
            double sum = 0.0;
            double sumSq = 0.0;
            foreach (Edge e in h.Values)
            {
        ...
        {
            Hashtable scoreOfVertex;

            internal CompareVerticesByScore(Hashtable scoreOfVertex)
            {
            ...
            {
                double scoreX = (double)scoreOfVertex[x];
                double scoreY = (double)scoreOfVertex[y];
                if (scoreX < scoreY)
                    return 1;
        ...
        {
            Hashtable scoreOfVertex = new Hashtable();
            ArrayList verticesSortedByScore = new ArrayList();
            foreach (Vertex v in graph.vertices.Values)
            {
                if (v.weight > 100*1000)
                {
...
//          vn.basicWeight = v.basicWeight;
        ...
        private void ShowWhoAllocated(string title, int allocatedAfterTickIndex, int allocatedBeforeTickIndex)
        {
            ...
            Histogram allocationHistogram = new Histogram(objectGraph.readNewLog);
            bool anyVertexSelected = SelectedVertexCount() != 0;
            foreach (ObjectGraph.GcObject gcObject in objectGraph.idToObject.Values)
            {
                ...
                {
                    bool thisVertexSelected = false;
                    if (anyVertexSelected)
                    {
                        gcObject.vertex = null;
                        Vertex v = objectGraph.FindVertex(gcObject, graph, ObjectGraph.BuildTypeGraphOptions.LumpBySignature);
                        thisVertexSelected = v.selected;
                    }
                    if (anyVertexSelected == thisVertexSelected)
                        allocationHistogram.AddObject(gcObject.typeSizeStackTraceId, 1);
                }
            }

            // Build the real graph from the histogram

            Graph g = allocationHistogram.BuildAllocationGraph();
        ...
        {
            ObjectGraph objectGraph = (ObjectGraph)graph.graphSource;
            Graph g = objectGraph.BuildTypeGraph(graph.previousGraphTickIndex, int.MaxValue, ObjectGraph.BuildTypeGraphOptions.LumpBySignature);
        ...
        private void showObjectsAllocatedBetween_Click(object sender, System.EventArgs e)
        {
            ...
            {
                ObjectGraph objectGraph = (ObjectGraph)graph.graphSource;
                Graph g = objectGraph.BuildTypeGraph(commentRangeForm.startTickIndex, commentRangeForm.endTickIndex, ObjectGraph.BuildTypeGraphOptions.LumpBySignature);
        ...
        {
            ObjectGraph objectGraph = (ObjectGraph)graph.graphSource;
            Graph g = objectGraph.BuildTypeGraph(graph.allocatedAfterTickIndex, graph.allocatedBeforeTickIndex, ObjectGraph.BuildTypeGraphOptions.IndividualObjects);

            string title = string.Format("Live Instances Allocated");
            GraphViewForm graphViewForm = new GraphViewForm(g, title);
            graphViewForm.Visible = true;
        }       
    }
