...
namespace CLRProfiler
{
	...
	internal class CallTreeForm : System.Windows.Forms.Form, IComparer, ITreeOwner
	{
		...
		private string[] names, types, signatures;
		...
		private TreeListView callTreeView;
		...
		private int[] IndexToStacktrace(int stackid)
		{
			if(stackid < 0)
			{
		...
		private void InitForm( bool fFirstTime )
		{
			...
			defaultFont = Form1.instance.font;
		...
		int GetFunctionIdFromStackId(int stackid)
		{
			if(stackid == -1)
			{
				return -1;
		...
		void UpdateStats(object r, object k)
		{
			...
			root.data.numberOfFunctionsCalledByKids += kid.data.numberOfFunctionsCalled;
		...
		bool BuildTreeAndComputeStats()
		{
				...
				while (c != -1)
				{
					...
					if(threadid != prevThreadId)
					{
						if(prevThreadId != -1)
						{
							...
							ThreadState prevState = (ThreadState)threads[prevThreadId];
						...
						ThreadState state = (ThreadState)threads[threadid];
						if(state == null)
						{
							/* create if necessary */
					...
					if ( nodetype == TreeNode.NodeType.Allocation )
					{
						...
						for ( ; i < curStackLen; i++)
						{
							...
							stacktraceTable.Add( nextStackIndex, stacktrace, 2, i+1);
			...
			foreach(object key in threads.Keys)
			{
				if(threads[key] == null)
				{
					nulls.Add(key);
		...
		private void threadIDList_SelectedValueChanged(object sender, System.EventArgs e)
		{
			...
			if(selectedThread != currentThreadId)
			{
				...
				callTreeView = ((ThreadState)threads[selectedThread]).callTreeView;
