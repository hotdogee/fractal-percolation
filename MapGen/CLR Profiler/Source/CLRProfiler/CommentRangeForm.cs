...
namespace CLRProfiler
{
    ...
    public class CommentRangeForm : System.Windows.Forms.Form
    {
        ...
        private int[] commentTickIndex;

        private void FillComboBoxes()
        {
            SampleObjectTable.SampleObject commentList = Form1.instance.lastLogResult.sampleObjectTable.commentList;
            
            string[] commentTable = Form1.instance.lastLogResult.sampleObjectTable.readNewLog.commentTable;

            int commentCount = 0;
            for (SampleObjectTable.SampleObject cl = commentList; cl != null; cl = cl.prev)
                commentCount++;

            string[] comments = new String[commentCount];
            commentTickIndex = new int[commentCount];

            for (SampleObjectTable.SampleObject cl = commentList; cl != null; cl = cl.prev)
            {
                commentCount--;
                comments[commentCount] = commentTable[cl.typeIndex];
                commentTickIndex[commentCount] = cl.allocTickIndex;
            }

            startComboBox.Items.Add("Start of Application");
        ...
        private void okButton_Click(object sender, System.EventArgs e)
        {
            ...
            startTickIndex = -1;
            if (selectedIndex >= 1)
                startTickIndex = commentTickIndex[selectedIndex - 1];
            selectedIndex = endComboBox.SelectedIndex;
            endComment = (string)endComboBox.Items[selectedIndex];
            endTickIndex = int.MaxValue;
            if (selectedIndex < commentTickIndex.Length)
                endTickIndex = commentTickIndex[selectedIndex];
        }
    }
}
