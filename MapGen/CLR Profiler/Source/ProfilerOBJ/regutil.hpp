                              ...
                              const char *szValue )
{
    ...
    if ( szSubkey != NULL )
    {
        ...
        char *szFullKey = (char *)_alloca(strlen(szKey) + 1 + strlen(szSubkey) + 1);
        if ( szFullKey == NULL )
            return FALSE;
                                   ...
                                   const char *szModule ) 
{
    ...
    _snprintf( rcIndProgID, NumItems(rcIndProgID)-1, "%s.%s", szProgIDPrefix, szClassProgID ) ;
    rcIndProgID[NumItems(rcIndProgID)-1] = 0;
    _snprintf( rcProgID, NumItems(rcProgID)-1, "%s.%d", rcIndProgID, iVersion );
    ...
    if ( SUCCEEDED( hr ) )
    {
        ...
        _snprintf( rcInproc, NumItems(rcInproc)-1, "%s\\%s", rcCLSID, "InprocServer32" );
        rcInproc[NumItems(rcInproc)-1] = 0;
                                     ...
                                     const char *szClassProgID )
{
    ...
    // format the prog ID values.
    _snprintf( rcProgID, NumItems(rcProgID)-1, "%s.%s", szProgIDPrefix, szClassProgID );
    rcProgID[NumItems(rcProgID)-1] = 0;
    _snprintf( rcIndProgID, NumItems(rcIndProgID)-1, "%s.%d", rcProgID, iVersion );
    rcIndProgID[NumItems(rcIndProgID)-1] = 0;
