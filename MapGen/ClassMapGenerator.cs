...
namespace MapGen
{
    public class MapGeneratorInput
    {
        ...
        private string _fromFile = ""; // fractal
        ...
        public MapGeneratorInput(string fromFile, int width, int height)
        {
            Width = width;
            Height = height;
            FromFile = fromFile;
    ...
    public class MapGenerator
    {
        ...
        public MapGeneratorInput Input
        {
            ...
            {
                _input = value;
                _byteMap = new byte[value.Width][];
                for (int i = 0; i < value.Width; i++)
                {
                    _byteMap[i] = new byte[value.Height];
                }
            }
        }
        public byte[][] ByteMap
        ...
        // Save and Read maps as XML
        public void Save(string fileName)
        {
            XmlSerializer serializer = new XmlSerializer(typeof(byte[][]));
            TextWriter textWriter = new StreamWriter(fileName);
            serializer.Serialize(textWriter, _byteMap);
            textWriter.Close();
        }
        ...
        public void Read(string fileName)
        {
            ...
            _byteMap = (byte[][])deserializer.Deserialize(textReader);
            textReader.Close();
        }

        public static void SaveMapText(string fileName, byte[][] byteMap, string sep = " ")
        {
            ...
            for (int y = 0; y < height; y++)
            {
                ...
                {
                    strRow.Add(byteMap[x][y] == 1 ? "2" : "1");
                }
                sw.WriteLine(string.Join<string>(sep, strRow));
            }
            sw.Close();
        }

        public static byte[][] ReadMapText(string fileName, string passableLabels="1", char sep=' ')
        {
            List<string> labels = new List<string>(passableLabels.Split(' ', ','));
            char[] sepa = { sep };
            ...
            {
                strMap.Add(line.Split(sepa, StringSplitOptions.RemoveEmptyEntries));
            }
            sr.Close();
            int height = strMap.Count;
            int width = strMap[0].Length;
            byte[][] byteMap = new byte[width][];
            for (int i = 0; i < width; i++)
            {
                byteMap[i] = new byte[height];
            }
            for (int y = 0; y < height; y++)
            {
                for (int x = 0; x < width; x++)
                {
                    byteMap[x][y] = labels.Contains(strMap[y][x]) ? (byte)0 : (byte)1;
                }
            }
            return byteMap;
        }

        public static void SaveMap(string fileName, byte[][] byteMap)
        {
            XmlSerializer serializer = new XmlSerializer(typeof(byte[][]));
            TextWriter textWriter = new StreamWriter(fileName);
            serializer.Serialize(textWriter, byteMap);
            textWriter.Close();
        }

        public static byte[][] ReadMap(string fileName)
        {
            XmlSerializer deserializer = new XmlSerializer(typeof(byte[][]));
            TextReader textReader = new StreamReader(fileName);
            byte[][] byteMap = (byte[][])deserializer.Deserialize(textReader);
            textReader.Close();
            return byteMap;
        }
    }
